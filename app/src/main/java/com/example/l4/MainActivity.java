...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        viewTracksButton.setOnClickListener(v -> {
            Intent intent = new Intent(MainActivity.this, com.example.l4.TrackListActivity.class);
            startActivity(intent);
        });

        if (!isInternetAvailable(this)) {
            Toast.makeText(this, "Запущено в автономном режиме", Toast.LENGTH_SHORT).show();
        } else {
            startPolling();
        }
    }
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                if (isInternetAvailable(getApplicationContext())) {
                    com.example.l4.SongFetcher songFetcher = new com.example.l4.SongFetcher(db, getApplicationContext());
